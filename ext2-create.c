...
void write_block_bitmap(int fd) {
	...
	for (int i = NUM_BLOCKS; i <= 8 * NUM_BLOCKS; i++) {
		bitmap[(i - 1) / 8] |= 1 << (i - 1) % 8;
	}
	/* This is all you */
...
void write_inode_table(int fd) {
	...
	// hello-world sym directory
	struct ext2_inode hello_world_sym_inode = {0};
	hello_world_sym_inode.i_mode = EXT2_S_IFLNK
								  | EXT2_S_IRUSR
								  | EXT2_S_IWUSR
								  | EXT2_S_IRGRP
								  | EXT2_S_IROTH;
	...
	hello_world_sym_inode.i_dtime = 0;
